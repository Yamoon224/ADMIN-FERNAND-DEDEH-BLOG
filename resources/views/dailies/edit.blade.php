            ...
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                <div class="flex-grow-1">
                    <h1 class="h3 fw-bold mb-1">
                        @lang('locale.daily', ['suffix'=>app()->getLocale() == 'en' ? 'ies' : 's'])
                    </h1>
                </div>
                <nav class="flex-shrink-0 mt-1 mt-sm-0 ms-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        ...
                        <li class="breadcrumb-item" aria-current="page">
                            @lang('locale.edit', ['param' => __('locale.daily', ['suffix'=>app()->getLocale() == 'en' ? 'y' : ''])])
                        </li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default bg-primary text-white">
                <h3 class="block-title">
                    @lang('locale.form', ['param'=>__('locale.edit', ['param'=>__('locale.daily', ['suffix'=>app()->getLocale() == 'en' ? 'y' : ''])])])
                </h3>
            </div>
                                        ...
                                        <div class="hashtag-row" data-index="{{ $index }}" data-existing="true" data-content-id="{{ $content->id }}">
                                            <div class="row align-items-start border-bottom border-warning mb-4 p-4" style="border-bottom-width:2px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); border-radius: 0.5rem;">
                                                ...
                                                <div class="col-12 text-end mt-2">
                                                    <button type="button" class="btn btn-xs btn-danger removeHashtag">
                                                        <i class="si si-trash"></i> @lang('locale.delete')
                                                    </button>
                                                </div>
                                            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById('addHashtag').addEventListener('click', function () {
                ...
                delete newRow.dataset.contentId;
                ...
                // Ajout bouton supprimer pour le nouveau
                if (!newRow.querySelector('.removeHashtag')) {
                    const removeBtn = document.createElement('button');
                    removeBtn.type = "button";
                    removeBtn.className = "btn btn-xs btn-danger removeHashtag mt-2";
                    removeBtn.innerHTML = '<i class="si si-trash"></i> @lang("locale.delete")';
