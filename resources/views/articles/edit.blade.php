        ...
        <div class="content">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                <div class="flex-grow-1">
                    <h1 class="h3 fw-bold mb-1">@lang('locale.article', ['suffix' => ''])</h1>
                </div>
                <nav class="flex-shrink-0 mt-1 mt-sm-0 ms-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        <li class="breadcrumb-item">
                            <a class="link-fx" href="{{ route('articles.index') }}">@lang('locale.posting')</a>
                        </li>
                        <li class="breadcrumb-item" aria-current="page">@lang('locale.edit', ['param' => __('locale.article', ['suffix' => ''])])</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">@lang('locale.form', ['param'=>__('locale.edit', ['param'=>__('locale.article', ['suffix' => ''])])])</h3>
            </div>
                    ...
                    <div class="row">
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="type" name="type" required>
                                    ...
                                    @foreach (['ARTICLE', 'PODCAST'] as $item)
                                        <option value="{{ $item }}" {{ $article->type == $item ? 'selected' : '' }}>{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>                          
                        </div>
                        ...
                        <div class="col-12">
                            <div class="mb-4">
                                ...
                                <textarea class="form-control form-control-alt" id="title" name="title" placeholder="@lang('locale.title')" required>{{ $article->title }}</textarea>
                            </div>
                        
                            <div class="mb-4" id="file-wrapper">
                                <label class="form-label">@lang('locale.article_image')</label>
                                <input 
                                    type="file" 
                                    name="path_file" 
                                    id="uploaded_file" 
