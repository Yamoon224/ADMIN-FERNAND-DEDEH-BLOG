        ...
        <div class="block block-rounded overflow-hidden">
            <div class="block-header block-header-default d-flex align-items-center justify-content-between">
                <div class="block-title">
                    <ul class="nav nav-tabs nav-tabs-block" role="tablist">
                        <li class="nav-item">
                            <button type="button" class="nav-link active" id="horizontal_banners-tab"
                                data-bs-toggle="tab" data-bs-target="#horizontal_banners" role="tab"
                                aria-controls="horizontal_banners" aria-selected="true">@lang('locale.horizontal_banners')</button>
                        </li>
                        <li class="nav-item">
            ...
            <div class="block-content tab-content overflow-hidden">
                <div class="tab-pane fade fade-up show active" id="horizontal_banners" role="tabpanel"
                    ...
                    @session('message')
                        <div class="fs-4 fw-semibold p-2 mb-4 border-start border-4 border-success bg-body-light">
                            {{ session('message') }}</div>
                    @endsession

                    @foreach ($horizontales as $item)
                        <div class="block block-rounded bg-transparent bg-image"
                            ...
                            <div class="block-content block-content-full bg-primary-dark-op">
                                <div class="py-4 text-center">
                                    ...
                                    <div class="d-flex justify-content-center gap-2">
                                        <a role="button" class="btn btn-sm btn-primary" data-id="{{ $item->id }}"
                                            data-link="{{ $item->link }}" data-position="{{ $item->position }}"
                            ...
                            <nav aria-label="Projects Search Navigation">
                                <ul class="pagination pagination-sm">
                                    ...
                                    <li class="page-item {{ $horizontales->onFirstPage() ? 'disabled' : '' }}">
                                        <a class="page-link"
                                            href="{{ $horizontales->previousPageUrl() ?? 'javascript:void(0)' }}"
                                            aria-label="Previous">
                                    ...
                                    @foreach ($horizontales->getUrlRange(1, $horizontales->lastPage()) as $page => $url)
                                        <li
                                            class="page-item {{ $horizontales->currentPage() == $page ? 'active' : '' }}">
                                    ...
                                    <li class="page-item {{ $horizontales->hasMorePages() ? '' : 'disabled' }}">
                                        <a class="page-link"
                                            href="{{ $horizontales->nextPageUrl() ?? 'javascript:void(0)' }}"
                                            aria-label="Next">
                                ...
                                <a class="block block-rounded bg-image h-100 mb-0"
                                    ...
                                    <div class="block-content bg-black-50">
                                        <div
                                            ...
                                            <p>
                                                <span
                                                    class="badge bg-primary fw-bold p-2 text-uppercase">{{ $item->position }}</span>
                                            </p>
                                            <p class="fs-sm">
                                        ...
                                        <p class="fw-medium text-white-75"><a
                                                href="{{ $item->link }}">@lang('locale.link')</a></p>
                                    </div>
                                </a>
                            </div>
                        @endforeach
                    ...
                    <form method="POST" id="edit-banner-form" enctype="multipart/form-data">
                        ...
                        <div class="block-content fs-sm">
                            <div class="row">
                                <div class="col-12">
                                    <div class="mb-4">
                                        <label class="form-label" for="edit-banner-link">@lang('locale.behind_link') <span
                                                class="text-danger">*</span></label>
                                        <input type="text" class="form-control form-control-alt"
                                            id="edit-banner-link" name="link" required>
                                    </div>
                                    <div class="mb-4">
                                        <label class="form-label" for="edit-banner-position">@lang('locale.position') <span
                                                class="text-danger">*</span></label>
                                        <select class="form-select" id="edit-banner-position" name="position"
                                            required>
                                            <option value="">-- @lang('locale.select') --</option>
                                            @foreach ([
        'HEADER' => 'En-tête',
